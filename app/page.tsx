    ...
    const fetchData = async () => {
      try {
        ...
        const upcomingEvents = allEvents.filter((event: any) => {
          const eventDate = new Date(event.dateEvent + " " + (event.strTime || "00:00:00"));
    ...
    <main className="min-h-screen bg-[#020617] text-white p-4 max-w-xl mx-auto pb-32">
        ...
        <div className="flex overflow-x-auto gap-2 pb-2 no-scrollbar">
          {LEAGUES.map((league) => (
            <button
              ...
              className={`whitespace-nowrap px-4 py-2 rounded-xl text-[10px] font-bold border ${
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          return (
            <div key={item.fixture.id} className={`p-6 rounded-3xl border ${live ? "bg-red-900/30 border-red-500" : "bg-[#0f172a] border-slate-800"}`}>
              
              <div className="text-[10px] mb-3 font-bold flex justify-between uppercase tracking-widest">
                <span className="text-blue-400">{item.league.name}</span>
                <span className="text-slate-400">
                  {getDayLabel(item.date)} • {formatLocalTime(item.date, item.time)}
                </span>
                <span className={`${live ? "text-red-400 animate-pulse" : "text-emerald-500"}`}>
                  {live ? "LIVE" : `AI: ${getAutoPick(probs)}`}
                </span>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  {["BTTS","Overs_Unders","Double_Chance","Handicap","Clean_Sheet","First_Half","Home_Overs","Total_Corners"].map((m) => (
                    <div key={m} className="p-3 bg-black/40 border border-slate-800 rounded-xl" onClick={() => !isPaid && setShowPaymentModal(true)}>
                      ...
                      <p className={`text-xs font-black ${isPaid ? "text-blue-400" : "blur-sm opacity-50"}`}>

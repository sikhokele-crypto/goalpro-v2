...
export default function GoalPro() {
  ...
  const poisson = (expected: number, actual: number) => {
    return (Math.exp(-expected) * Math.pow(expected, actual)) / factorial(actual);
  };
  ...
  const getPoissonPredictions = (item: any) => {
    ...
    // Simulate Lambda (Expected Goals) based on Team Power
    const homeLambda = ((hId % 10) / 4) + 1.2; // Avg 1.2 - 3.5 goals
    const awayLambda = ((aId % 10) / 5) + 0.8; // Avg 0.8 - 2.8 goals
  ...
  const getAutoPick = (probs: any) => {
    if (probs.homeProb > probs.awayProb && probs.homeProb > 45) return "HOME WIN";
    if (probs.awayProb > probs.homeProb && probs.awayProb > 45) return "AWAY WIN";
    if (probs.drawProb > 30) return "DRAW / X";
    return "BTTS YES";
  };
  // -----------------------------------------------------------------------
  ...
  useEffect(() => {
    const fetchLiveData = async () => {
      try {
        ...
        const validMatches = (res.data.response || []).filter((f: any) => 
  ...
  const getEliteMarket = (item: any, market: string, probs: any) => {
    const markets: any = {
      "BTTS": (probs.homeLambda > 1.5 && probs.awayLambda > 1.2) ? "Yes" : "No",
      "Overs_Unders": (probs.homeLambda + probs.awayLambda > 2.5) ? "Over 2.5" : "Under 2.5",
      ...
      "Clean_Sheet": probs.awayLambda < 1.0 ? "Home Yes" : "No",
      "First_Half": probs.homeProb > 40 ? "Home" : "Draw",
      "Home_Overs": `Over ${probs.homeLambda > 1.8 ? '1.5' : '0.5'}`
    };
    return markets[market] || "88% IQ";
  };
  ...
  const AdSlot = () => {
    ...
    return (
      <div className="my-6 p-4 bg-slate-900/40 rounded-3xl border border-dashed border-slate-800 text-center min-h-[100px]">
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100 p-4 font-sans max-w-xl mx-auto pb-32">
      ...
      <header className="sticky top-0 z-40 bg-[#020617]/95 backdrop-blur-md pt-4 pb-6 border-b border-slate-800/50 mb-8 px-2">
        ...
        <input type="text" placeholder="Search Teams or Leagues..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-[#0f172a] border border-slate-800 rounded-2xl py-4 px-6 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
      </header>

      <div className="space-y-8">
        {loading ? (
           <p className="text-center text-blue-500 animate-pulse font-black uppercase text-[10px] py-20">Running Poisson Calculations...</p>
          ...
          filteredFixtures.slice(0, 50).map((item: any, index: number) => {
            ...
            return (
              <div key={item.fixture.id}>
                {index % 5 === 0 && <AdSlot />}
                <div className="bg-[#0f172a] rounded-[2.5rem] border border-slate-800/80 p-6 shadow-2xl relative overflow-hidden">
                  {/* AUTO-PICK BADGE */}
                  <div className="absolute top-0 right-10 bg-blue-600 px-4 py-1.5 rounded-b-xl shadow-lg">
                    <p className="text-[7px] font-black uppercase tracking-widest text-white">Auto-Pick: <span className="text-yellow-300">{autoPick}</span></p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button onClick={() => setSelectedMatch(selectedMatch === item.fixture.id ? null : item.fixture.id)} className="py-4 text-[9px] font-black text-white uppercase bg-blue-600/10 border border-blue-500/20 rounded-2xl">
                      {selectedMatch === item.fixture.id ? "Close IQ ▲" : "View IQ Analysis ▼"}
                    </button>
                  ...
                  {selectedMatch === item.fixture.id && (
                    <div className="mt-4 pt-4 border-t border-slate-800/50 grid grid-cols-2 gap-3">
                      {["BTTS", "Overs_Unders", "Total_Corners", "Double_Chance", "Handicap", "Clean_Sheet", "First_Half", "Home_Overs"].map((m) => (
                        <div key={m} onClick={() => !isPaid && setShowPaymentModal(true)} className="p-4 rounded-2xl border border-slate-800/50 bg-slate-900/30">
                          <p className="text-[8px] text-slate-400 font-black uppercase mb-1">{m.replace('_', ' ')}</p>
                          <div className="flex justify-between items-center">
                            ...
                            {!isPaid && <span className="text-[6px] bg-blue-600 px-2 py-0.5 rounded-full font-black">VIP</span>}
                          </div>
                        </div>
                      ))}
                    </div>

...
export default function GoalPro() {
  ...
  // 🔹 MODIFIED getAutoPick to respect isPaid
  const getAutoPick = (probs: any, isPaid: boolean) => {

    // 🔒 UNPAID USERS → ONLY HOME/DRAW/AWAY
    if (!isPaid) {
      if (probs.homeProb > probs.awayProb && probs.homeProb > probs.drawProb)
        return "HOME WIN";
      if (probs.awayProb > probs.homeProb && probs.awayProb > probs.drawProb)
        return "AWAY WIN";
      return "DRAW";
    }

    // 🔓 PAID USERS → original logic
      ...
      <div className="space-y-8">
        {loading ? (
          ...
          fixtures.filter((f: any) => 
            ...
            const autoPick = getAutoPick(probs, isPaid);
            const isPopular = POPULAR_LEAGUES.includes(item.league.id);

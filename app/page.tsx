  ...
  useEffect(() => {
    const fetchFixtures = async () => {
      try {
        ...
        // Added a headers configuration to specifically handle the API Sports requirements
        const config = {
          method: 'get',
        ...
        // If today is empty (common at midnight), try fetching "live" status matches globally
        if (data.length === 0) {
          const liveRes = await axios({
            ...config,
            url: 'https://v3.football.api-sports.io/fixtures?live=all'
          });
          setFixtures(liveRes.data.response || []);
        } else {
          setFixtures(data.filter(f => f.fixture.status.short !== 'FT'));
        }
      ...
      <header className="sticky top-0 z-40 bg-[#020617]/90 backdrop-blur-xl pt-4 pb-6 border-b border-white/5 mb-8">
        <div className="flex justify-between items-center mb-6 px-2">
          ...
          <button 
            ...
            className={`px-6 py-2 rounded-2xl text-[10px] font-black uppercase ${isPaid ? 'bg-emerald-500' : 'bg-blue-600 shadow-lg shadow-blue-600/20'}`}
          >
        ...
        <input 
          ...
          className="w-full bg-slate-900/50 border border-white/10 rounded-2xl py-4 px-6 text-sm focus:outline-none focus:border-blue-500/50"
        />
      </header>
      ...
      <div className="space-y-6">
        {loading ? (
           <div className="text-center py-20 animate-pulse">
             <p className="text-[10px] font-black uppercase text-blue-500 tracking-widest">Initialising IQ Engine...</p>
           </div>
          ...
          <div className="text-center py-20 bg-slate-900/20 rounded-[3rem] border border-dashed border-white/5">
            ...
            <button onClick={() => window.location.reload()} className="mt-4 text-blue-500 text-[10px] font-black underline">REFRESH FEED</button>
          </div>
        ) : (
            ...
            <div key={item.fixture.id} className="bg-slate-900/40 rounded-[2.5rem] border border-white/5 p-6 shadow-2xl">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button 
                  ...
                  className="py-4 bg-white/5 rounded-2xl text-[9px] font-black uppercase"
                >
                ...
                <div className="mt-4 pt-4 border-t border-white/5 grid grid-cols-2 gap-3">
                   {['BTTS IQ', 'OVER 2.5', '1X2 SAFE', 'CORNERS'].map(market => (
                     <div key={market} onClick={() => !isPaid && setShowPaymentModal(true)} className="p-4 bg-black/20 rounded-2xl border border-white/5 cursor-pointer">
                        ...
                        <p className={`text-xs font-black ${isPaid ? 'text-blue-400' : 'blur-sm opacity-30'}`}>{isPaid ? "READY" : "LOCKED"}</p>
                     </div>
                   ))}
                </div>
              )}
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-50 flex items-center justify-center p-6">
          <div className="bg-slate-900 border border-white/10 rounded-[3rem] p-8 w-full max-w-sm text-center">
            ...
            <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-8">Global Access Unlocked</p>
            ...
            <button onClick={() => setShowPaymentModal(false)} className="mt-6 text-slate-500 text-[10px] font-black uppercase tracking-widest">Cancel</button>
          </div>
        </div>
      )}
    </main>

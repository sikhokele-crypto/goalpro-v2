    ...
    <main className="min-h-screen bg-[#020617] text-slate-100 p-4 max-w-xl mx-auto pb-40">
      ...
      <header className="sticky top-0 bg-[#020617]/95 pt-4 pb-6 mb-8 z-30 border-b border-slate-900">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => !isPaid && setShowPaymentModal(true)} className={`${isPaid ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/50' : 'bg-blue-600 text-white shadow-lg shadow-blue-900/40'} px-5 py-2 rounded-2xl text-[10px] font-black tracking-widest transition-all hover:scale-105 active:scale-95`}>
            {isPaid ? "VIP ACTIVE" : "UPGRADE"}
          </button>
        </div>
        <input placeholder="Search team or league..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full p-4 rounded-2xl bg-[#0f172a] border border-slate-800 text-sm focus:ring-2 focus:ring-blue-600 outline-none" />
      </header>
              ...
              return (
                <div key={item.fixture.id}>
                  <div className="bg-[#0f172a] rounded-[2.5rem] border border-slate-800/60 overflow-hidden shadow-2xl p-6">
                    <div className="flex justify-between items-center mb-6">
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{item.league.name}</span>
                    ...
                    <div className="bg-blue-600 rounded-2xl p-4 mb-4 flex justify-between items-center shadow-lg shadow-blue-900/20">
                      ...
                      <span className="text-sm font-black text-white">
                        {probs.homeProb > probs.awayProb && probs.homeProb > probs.drawProb ? "HOME WIN" : 
                         probs.awayProb > probs.homeProb && probs.awayProb > probs.drawProb ? "AWAY WIN" : "DRAW / X"}
                      </span>
                    </div>

                    <button onClick={() => setSelectedMatch(selectedMatch === item.fixture.id ? null : item.fixture.id)} className="w-full bg-slate-800/40 hover:bg-slate-800 p-3 rounded-xl text-[10px] font-black tracking-widest text-slate-300 border border-slate-700/50 transition-colors">
                    ...
                    {selectedMatch === item.fixture.id && (
                      <div className="grid grid-cols-2 gap-2 mt-4 animate-in fade-in zoom-in duration-200">
                        {["BTTS","Overs_Unders","Double_Chance","Handicap","Clean_Sheet","First_Half","Home_Overs","Total_Corners"].map(m => (
                          <div key={m} onClick={() => !isPaid && setShowPaymentModal(true)} className="p-4 bg-black/40 rounded-2xl border border-slate-800/50 cursor-pointer group hover:border-blue-500/50 transition-all">
                    ...
                    <a href={BETWAY_AFFILIATE_URL} target="_blank" className="block text-center mt-6 text-[10px] font-bold text-slate-600 uppercase tracking-widest underline decoration-slate-800 underline-offset-4 hover:text-blue-500 transition-colors">Bet on this match at Betway →</a>
                  </div>
                </div>
              );
            })
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-6 z-50 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-[#0f172a] p-8 rounded-[3rem] text-center border border-blue-600/30 max-w-sm w-full shadow-2xl">
            ...
            <p className="text-slate-400 text-xs mb-8">Unlock 8+ Elite Markets and High-Probability VIP slips instantly.</p>
            <div id="paypal-button-container" className="min-h-[150px]"></div>
            <button onClick={() => setShowPaymentModal(false)} className="mt-6 text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] hover:text-white transition-colors">Maybe Later</button>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-8 left-0 right-0 px-6 z-40">
        <nav className="max-w-xs mx-auto bg-blue-600 p-2 rounded-[2rem] flex justify-between items-center shadow-2xl border border-blue-400/30">
          ...
          <a href={BETWAY_AFFILIATE_URL} target="_blank" className="flex-1 text-white/60 font-black text-[10px] tracking-widest text-center hover:text-white transition-colors">BET</a>
        </nav>
      </div>
    </main>
  );

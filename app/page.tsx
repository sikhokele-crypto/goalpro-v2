...
export default function GoalPro() {
  ...
  const getEliteMarket = (item: any, market: string, probs: any) => {
    const markets: any = {
      ...
      "CORNERS": `OVER ${(item.teams.home.id % 4) + 7.5}`,
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100 p-4 font-sans max-w-xl mx-auto pb-32">
      ...
      <div className="space-y-8">
        {loading ? (
           <p className="text-center text-blue-500 animate-pulse font-black uppercase text-[10px] py-20">Calculating Poisson IQ...</p>
        ) : (
          fixtures.filter((f: any) => 
            ...
            return (
              <div key={item.fixture.id} className={`bg-[#0f172a] rounded-[2.5rem] border ${isPopular ? 'border-blue-500/40' : 'border-slate-800/80'} p-6 relative overflow-hidden`}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => setSelectedMatch(selectedMatch === item.fixture.id ? null : item.fixture.id)} className="py-4 text-[9px] font-black text-white uppercase bg-blue-600/10 border border-blue-500/20 rounded-2xl">
                    {selectedMatch === item.fixture.id ? "Close Stats ▲" : "Elite Predictions ▼"}
                  </button>
                ...
                {selectedMatch === item.fixture.id && (
                  <div className="mt-4 pt-4 border-t border-slate-800/50 grid grid-cols-2 gap-3">
                    {["BTTS", "OVERS_UNDERS", "DOUBLE_CHANCE", "HANDICAP", "CLEAN_SHEET", "FIRST_HALF", "HOME_OVERS", "CORNERS"].map((m) => (
      ...
      {showPaymentModal && (
        <div className="fixed inset-0 bg-black/98 backdrop-blur-2xl flex items-center justify-center p-6 z-50">
          <div className="bg-[#0f172a] border border-blue-500/20 rounded-[3rem] p-10 w-full max-w-sm text-center">
            <h2 className="text-3xl font-black italic mb-6 uppercase text-white">Unlock VIP</h2>

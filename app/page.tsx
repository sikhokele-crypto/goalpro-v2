...
export default function GoalPro() {
  ...
  // HYDRATION FIX: Prevents the "White Screen" on Vercel
  ...
  const getAutoPick = (probs: any) => {
    // 1. If PAID: Unlock Elite Predictions (Overs/BTTS)
    if (isPaid) {
      if (probs.homeLambda + probs.awayLambda > 2.7) return "OV 2.5 GOALS";
      if (probs.homeLambda > 1.7 && probs.awayLambda > 1.4) return "BTTS - YES";
    }

    // 2. If FREE: Strictly Match Result (1X2)
  ...
  if (!hasMounted) return null; // CRITICAL: Fixes Vercel blank screen
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100 p-4 font-sans max-w-xl mx-auto pb-32">
      ...
      <div className="space-y-8">
        {loading ? (
           <p className="text-center text-blue-500 animate-pulse font-black uppercase text-[10px] py-20 tracking-widest">Processing Live Data...</p>
        ) : (
              ...
              <div key={item.fixture.id}>
                ...
                <div className={`bg-[#0f172a] rounded-[2.5rem] border ${isPopular ? 'border-blue-500/40 shadow-blue-500/5' : 'border-slate-800/80'} p-6 shadow-2xl relative overflow-hidden`}>
                  ...
                  <div className="mb-8 px-1">
                    <div className="flex justify-between mb-2 text-[9px] font-black uppercase text-slate-500">
                      ...
                      {["BTTS", "Overs_Unders", "Double_Chance", "Handicap", "Clean_Sheet", "First_Half", "Home_Overs", "Total_Corners"].map((m) => (
                        <div key={m} onClick={() => !isPaid && setShowPaymentModal(true)} className="p-4 rounded-2xl border border-slate-800 bg-black/20 cursor-pointer">
                          ...
                          <div className="flex justify-between items-center">
                            <p className={`font-black text-xs ${!isPaid ? 'blur-md opacity-20' : 'text-blue-400 animate-in fade-in'}`}>

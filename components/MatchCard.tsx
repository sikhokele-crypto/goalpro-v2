...
export default function MatchCard({ match, isPaid, onUpgrade }: MatchCardProps) {
  ...
  return (
    <motion.div
      ...
      className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-gradient-to-br from-[#050816] via-[#0f172a] to-[#111827] shadow-2xl mb-12"
    >
      <div className="relative z-10">
        <div className="p-8 border-b border-white/10">
          <div className="flex justify-between items-center mb-8">
            <div className="space-y-2">
              <span className="text-[10px] bg-blue-500/10 text-blue-400 border border-blue-400/20 px-4 py-1.5 rounded-full font-black uppercase tracking-widest">
                {match.league}
              </span>
          ...
          <div className="flex justify-between items-center mb-10">
            ...
            <span className="text-slate-800 font-black text-5xl italic uppercase opacity-40">VS</span>
        ...
        <div className="relative p-8 pt-6 bg-black/20">
          {!isPaid && (
            <div className="absolute inset-0 z-30 flex flex-col items-center justify-center text-center px-8 bg-[#050816]/80 backdrop-blur-md rounded-b-[2.5rem]">
              ...
              <button onClick={onUpgrade} className="w-full max-w-xs py-5 rounded-2xl bg-amber-500 text-black font-black uppercase tracking-widest">
                Upgrade to VIP
              </button>
            </div>
          )}
        ...
        <button className="w-full bg-[#1ed760] hover:bg-[#1db954] py-8 flex items-center justify-center gap-3 transition-all">
...
function TeamSection({ logo, name, side }: any) {
  return (
    <div className="flex flex-col items-center w-[40%]">
      <div className="w-20 h-20 rounded-full bg-white/5 border border-white/10 flex items-center justify-center p-4 mb-4 shadow-2xl">
        {logo ? <Image src={logo} alt={name} width={52} height={52} className="object-contain" /> : <Shield className="text-slate-700" size={36} />}
      </div>
      <p className="text-white font-black text-xl text-center truncate w-full mb-1">{name}</p>
...
function MarketBox({ val, label, delay }: any) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }} className="bg-white/5 border border-white/10 p-6 rounded-[2rem] text-center">
...
function VipItem({ label, val, icon }: any) {
  return (
    <div className="bg-white/5 border border-white/10 p-5 rounded-[1.8rem] flex flex-col justify-center min-h-[100px]">
